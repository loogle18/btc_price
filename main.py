...
title = "Model V7 (inputs - 4, look back - %d, look ahead - %d, epochs - %d, linear-adamax, 64, 50-50 new data)" % (n_per_in, n_per_out, epochs)
...
res = model.fit(training_data, shapes["close"]["y"], epochs=epochs, batch_size=32, validation_split=0.1, callbacks=[tboard_callback])
model.save(
    "models/%s" % title,
    overwrite=True,
...
for n in reversed(range(1, len(df_test) - n_per_out - n_per_in - 1 )):
    yhat = model.predict([
    ...
    # print("pure yhat: ", yhat)
    yhat = yhat.tolist()[0]
    # print("yhat.tolist()[0]: ", yhat)
    # Transforming values back to their normal prices
    yhat = scalers["close"].inverse_transform(np.array(yhat).reshape(-1,1)).tolist()[0][0]

    predicted_list.append(yhat)

    # Getting the actual values from the last available y variable which correspond to its respective X variable
    actual = scalers["close"].inverse_transform(shapes_test["close"]["y"][-n].reshape(-1,1)).tolist()[0][0]
